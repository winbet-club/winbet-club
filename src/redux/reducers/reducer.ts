...
export const loadAdditionalJackpots = createAction(
  ACTIONS.LOAD_ADDITIONAL_JACKPOTS
);
...
export const toggleMobileMenuDescriptionCasinoOpen = createAction(
  ACTIONS.TOGGLE_MOBILE_MENU_DESCRIPTION_OPEN
);
export const changeCasinoDescriptionNav = createAction(
  ACTIONS.CHANGE_CASINO_DESCRIPTION_NAV
);
export const changeMenuFullNavItemsList = createAction(
...
export const mainReducer = handleActions<IStore, any>(
  {
    ...
    [ACTIONS.SAVE_ONE_JACKPOT]: (
      ...
      ...state,
      jackpotsValues: state.jackpotsValues.map(
        (jackpot: any, index: number) => ({
          ...jackpot,
          value: payload[index]
        })
      )
    }),
    [ACTIONS.TOGGLE_MOBILE_MENU]: (state: IStore): IStore => ({
      ...state,
      isMobileMenuOpen: !state.isMobileMenuOpen
    }),
    [ACTIONS.TOGGLE_MOBILE_MENU_DESCRIPTION_OPEN]: (state: IStore): IStore => ({
      ...state,
    ...
    [ACTIONS.NULL_NAVS]: (state: IStore): IStore => ({
      ...state,
      menuNavItemsList: state.menuNavItemsList.map(item => ({
        ...item,
        isActive: false
